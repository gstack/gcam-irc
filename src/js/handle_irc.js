...
if (typeof window !== 'undefined') {
  ...
  window.gcam.addEventListeners();
  window.gcam.sendCommand = function(to, obj) {
    obj.gcam = "v1";
    app.io.emit("command", {server: "irc.rizon.net", command: "PRIVMSG "+to+" "+JSON.stringify(obj)});
  }
}
  ...
  switch (message.rawCommand) {
    ...
    case "JOIN":
      ...
      if(message.nick === app.irc.getActiveNick()) {
        ...
        if (typeof window !== 'undefined') {  window.gcamEvents.emit('adduser', {from: message.args[0], nick: message.nick}); }
      }
      break;
    ...
    case "PART":
      if(message.nick === server.get("nick")) {
        ...
        if (typeof window !== 'undefined') { window.gcamEvents.emit('removeuser', {from: message.args[0], nick: message.nick}); }
      }
      break;
